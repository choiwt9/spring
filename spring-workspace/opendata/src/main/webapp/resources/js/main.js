...
//onload 시점
$(function () {
	//btn1 아이디 속성을 가지 요소의 클릭 이벤트
	$("#btn1").click(function () {
		/*		//ajax 통신 : 요청 주소 - air.do, 요청 데이터 - {location: 선택된 지역 정보}
...
const getMoreData = (nextNo) => {
	// 더보기 버튼이 클릭 되었을 때 데이터를 추가로 요청
	getAirPollution("air.do", nextNo, (result) => {
		appendJsonData(result);
	});
}

const appendJsonData = (bodyData, result) => {
	const data = result.response.body.items;
	/*
	let bodyData = $("#ap-table tbody").html();//처음에는 tbody 부분이 비어 있을 것이고
	//데이터를 조회한 경우에는 조회된 데이터들(tr)이 있을 것임
	*/
...
const appendXMLData = (tbodyDate, result) => {
	const itemArr = $(result).find("item");	// [<item/>,<item/>, <item/>,...]
	...
	$("#ap-table + button").remove();
	const totalCount = getElement(result, 'totalCount');
	
	const pageNo = getElement(result, 'pageNo');
	if ( $("#ap-table tbody tr").length < totalCount.text() ) {
		$("#ap-table").after("<button class='btn btn-dark w-100' onclick='getMoreDataXML("+
								(parseInt(pageNo.text())+1)
							+")'>더 보기</button>");
	}
}

const getElement = (base, target) => {
	const findEle = $(base).find(target);

	return findEle.length > 0 ? $(findEle[0]) : null;
}